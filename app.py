...
DB_PASSWORD = os.environ.get("DB_PASSWORD", "muthu2006")
...
# ══════════════════════════════════════════════════════════════════════════════
# FLASK ROUTES — DASHBOARD STATS
# ══════════════════════════════════════════════════════════════════════════════

@app.route("/api/dashboard")
def dashboard():
    """Quick summary card data for the dashboard."""
    zones   = Zone.query.all()
    depots  = Depot.query.all()
    sups    = Supply.query.all()

    total_food   = sum(s.quantity for s in sups if s.supply_type == "food")
    total_water  = sum(s.quantity for s in sups if s.supply_type == "water")
    total_meds   = sum(s.quantity for s in sups if s.supply_type == "medicines")
    total_people = sum(z.population for z in zones)
    avg_severity = round(sum(z.severity for z in zones) / len(zones), 1) if zones else 0

    return jsonify({
        "zones":         len(zones),
        "depots":        len(depots),
        "supply_records": len(sups),
...
def export_all():
    """Export all DB data as a single JSON payload."""
    zones   = [z.to_dict()  for z in Zone.query.all()]
    depots  = [dp.to_dict() for dp in Depot.query.all()]
    sups    = [s.to_dict()  for s in Supply.query.all()]
    return jsonify({
